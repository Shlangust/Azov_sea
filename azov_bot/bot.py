...
async def moderator(callback: types.CallbackQuery, state: FSMContext):
    ...
    async def stg_i(callback: types.CallbackQuery, state: FSMContext):
        ...
        with connection.cursor() as cursor:
            try:
                cursor.execute("SELECT * FROM moderation WHERE NOT EXISTS (SELECT 1 FROM banned WHERE banned.id_i = "
                               "moderation.id_i);")
                rows = cursor.fetchall()
                c = max(rows[2], key=max)[0]
                cursor.execute(
                    "SELECT * FROM moderation WHERE NOT EXISTS (SELECT 1 FROM banned WHERE banned.id_i = "
                    "moderation.id_i);")
                rows = cursor.fetchall()
            except Exception as ex:
                print("ошибка:", ex)
